...
action == "validate" && NR > 1 && tree_label != "" && length(tree_label) > 15 { log_err("warning"); print "tree_label length in " CSVFILENAME " line " NR " should be less than 15 and was " length(tree_label) " " RS $0 RS; } 
action == "validate:summary" && NR > 1 && tree_label != "" && length(tree_label) > 15 { key=CSVFILENAME FS "tree_label" FS  "maxLength" FS "max length is: 15" FS "warning"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && sector_or_quarter != "" && length(sector_or_quarter) > 2 { key=CSVFILENAME FS "sector_or_quarter" FS  "maxLength" FS "max length is: 2" FS "warning"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && distance && !is_numeric(distance) { key=CSVFILENAME FS "distance" FS  "type" FS "value should match: /^(A|Aw|Pb|Bw|Ax|Dd|Dc|P|Pl|Pw|Pa|Pf|Pj|Px|Sw|Se|Sb|Sx|Fb|Fa|Fd|Lt|Lw|La|Ls|Du|Ms|No)$/" FS "warning"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && azimuth && !is_numeric(azimuth) { key=CSVFILENAME FS "azimuth" FS  "type" FS "value should match: /^(A|Aw|Pb|Bw|Ax|Dd|Dc|P|Pl|Pw|Pa|Pf|Pj|Px|Sw|Se|Sb|Sx|Fb|Fa|Fd|Lt|Lw|La|Ls|Du|Ms|No)$/" FS "warning"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
