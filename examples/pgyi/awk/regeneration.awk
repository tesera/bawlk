...
action == "validate:summary" && NR > 1 && regeneration_count && !is_numeric(regeneration_count) { key=CSVFILENAME FS "regeneration_count" FS  "type" FS "value should match: /^(A|Aw|Pb|Bw|Ax|Dd|Dc|P|Pl|Pw|Pa|Pf|Pj|Px|Sw|Se|Sb|Sx|Fb|Fa|Fd|Lt|Lw|La|Ls|Du|Ms|No)$/" FS "warning"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && regeneration_count != "" && regeneration_count < (species=="No"?0:1) { key=CSVFILENAME FS "regeneration_count" FS  "minimum" FS "value should be greater than: (species=="No"?0:1)" FS "warning"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && regeneration_count != "" && regeneration_count > 9999 { key=CSVFILENAME FS "regeneration_count" FS  "maximum" FS "value should be less than: 9999" FS "warning"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
# la fin
END {
    if (action == "validate:summary" && length(dupkeys) > 0) for (dup in dupkeys) { violation=CSVFILENAME FS "comnpany|company_plot_number|measurement_number|regeneration_plot_name|species" FS  "duplicate" FS dup " violates pkey" FS "error"; violations[violation] = dupkeys[dup]}
