...
NR == 1 && action == "validate:summary" { headers="company|company_plot_number|disturbance_number|disturbance_code|disturbance_year|disturbance_month|disturbance_day|disturbance_comment"; if (!are_headers_valid(headers)) { violations[CSVFILENAME FS "headers" FS  "names" FS "csv headers are invalid"]=1; exit 0; } }
...
action == "validate:summary" && NR > 1 && disturbance_number && !is_numeric(disturbance_number) { key=CSVFILENAME FS "disturbance_number" FS  "type" FS "max length is: 15"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_number != "" && disturbance_number < 1 { key=CSVFILENAME FS "disturbance_number" FS  "minimum" FS "value should be greater than: 1"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_number != "" && disturbance_number > 12 { key=CSVFILENAME FS "disturbance_number" FS  "maximum" FS "value should be less than: 12"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_code != "" && disturbance_code !~ /^(BU|DF|MI|MU|NDD|NDW|DC|HL|MLU|NDC|NDI|FI|DA)$/ { key=CSVFILENAME FS "disturbance_code" FS  "pattern" FS "value should match: /^(BU|DF|MI|MU|NDD|NDW|DC|HL|MLU|NDC|NDI|FI|DA)$/"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_year && !is_numeric(disturbance_year) { key=CSVFILENAME FS "disturbance_year" FS  "type" FS "value should match: /^(BU|DF|MI|MU|NDD|NDW|DC|HL|MLU|NDC|NDI|FI|DA)$/"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_code!="" && disturbance_year == "" { key=CSVFILENAME FS "disturbance_year" FS  "required" FS "value is required if disturbance_code!="""; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_year != "" && disturbance_year < 1900 { key=CSVFILENAME FS "disturbance_year" FS  "minimum" FS "value should be greater than: 1900"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_year != "" && disturbance_year > 2050 { key=CSVFILENAME FS "disturbance_year" FS  "maximum" FS "value should be less than: 2050"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_month && !is_numeric(disturbance_month) { key=CSVFILENAME FS "disturbance_month" FS  "type" FS "value should be less than: 2050"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_month != "" && disturbance_month < 1 { key=CSVFILENAME FS "disturbance_month" FS  "minimum" FS "value should be greater than: 1"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_month != "" && disturbance_month > 12 { key=CSVFILENAME FS "disturbance_month" FS  "maximum" FS "value should be less than: 12"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_day && !is_numeric(disturbance_day) { key=CSVFILENAME FS "disturbance_day" FS  "type" FS "value should be less than: 12"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_day != "" && disturbance_day < 1 { key=CSVFILENAME FS "disturbance_day" FS  "minimum" FS "value should be greater than: 1"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
action == "validate:summary" && NR > 1 && disturbance_day != "" && disturbance_day > 31 { key=CSVFILENAME FS "disturbance_day" FS  "maximum" FS "value should be less than: 31"; if(!violations[key]) { violations[key]=0; } violations[key]++; } 
...
# la fin
END {
    if (action == "validate:summary" && length(dupkeys) > 0) for (dup in dupkeys) { violation=CSVFILENAME FS "company|company_plot_number|disturbance_number" FS  "duplicate" FS dup " violates pkey"; violations[violation] = dupkeys[dup]}
